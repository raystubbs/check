...
(defn jar [_]
  (run! #(io/delete-file % true) (reverse (file-seq (File. "target"))))
      ...
      {:class-dir class-dir
       ...
       :pom-data [[:licenses
                   [:license
                    ...
                    [:distribution "repo"]]]
                  [:scm
                   [:url "https://github.com/raystubbs/check"]
                   [:connection "scm:git:https://github.com/raystubbs/check.git"]
                   [:developerConnection "scm:git:ssh://git@github.com:raystubbs/check.git"]]]})
    (b/copy-dir
      {:src-dirs ["src"]
